...
describe("EventModel Query resolvers", () => {
  ...
  describe("eventModelDetail", () => {
    it("returns the eventModel from the id", async () => {
      ...
      const context = { mongo }
      const response = await resolvers.Query.eventModelDetail(null, { id: "5a4a5eb6404da6d636078beb" }, context)
      expect(response).toEqual(eventModelData[0])
    })
    it("returns null if no eventModel with that id", async () => {
      const context = { mongo }
      const response = await resolvers.Query.eventModelDetail(null, { id: "5a4a5eb6404da6d6360734eb" }, context)
