...
export default {
  Query: {
    userMembershipData: async (_, __, { mongo: { Memberships }, user }) => {
      ...
      const data = {}
      data.memberships = await Memberships.find({ ownerId: ObjectId(user._id) })
        .sort({ end: -1 })
        .toArray()
      if (data.memberships) {
        data.wasMember = true
      ...
      if (data.wasMember) {
        ...
        if (lastMemberDayToNow < 0) {
          ...
          data.nextStart = moment(data.memberships[0].end)
            .add(1, "d")
            .format("YYYY-MM-DD")
      ...
      data.nextEnd = moment(data.nextStart)
        .add(1, "y")
        .subtract(1, "d")
        .format("YYYY-MM-DD")
      return data
    ...
    createMembership: async (_, data, context) => {
      ...
      if (overlappingMembership) {
        throw new ValidationError(
          `Previous membership overlapping (ending ${moment(
            overlappingMembership.end,
          ).fromNow()}).`,
          "start",
        )
      ...
      const response = await Memberships.insertOne(newMembership)
      newMembership._id = response.insertedId
