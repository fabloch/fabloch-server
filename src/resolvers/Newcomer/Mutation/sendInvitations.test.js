  ...
  describe("sendInvitations", () => {
    describe("success", () => {
      it("increments invitationSent and invitationSentAt", async () => {
        ...
        const context = { mongo, user: admin, mailer }
        const invitationInput = {
          newcomerIds: [newcomerData[2]._id.toString(), newcomerData[3]._id.toString()],
          message: "A custom message",
        }
        const response = await resolvers.Mutation.sendInvitations(
          null,
          { invitationInput },
          context,
        )
        expect(response).toMatchObject([{ invitationSentCount: 2 }, { invitationSentCount: 1 }])
        expect(moment(response[0].invitationSentAt).format("LL")).toMatch(
          moment()
            .utc()
            .format("LL"),
        )
        expect(moment(response[1].invitationSentAt).format("LL")).toMatch(
          moment()
            .utc()
            .format("LL"),
        )
